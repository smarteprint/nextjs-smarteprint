...
export async function generateMetadata({ params }) {
  ...
  if (!product) {
    return {
      ...
      description: 'The requested product could not be found on Smart ePrint.',
    };
  }

  const title = product.title;
  const description = product.shortDetails || product.description || `Buy ${product.title} online at Smart ePrint. Get genuine printer quality, fast shipping, and expert support.`;
  const categoryName = product.category?.name || 'Printers';
  const imageUrl = product.images && product.images.length > 0 ? product.images[0] : 'https://www.smarteprint.com/smart-e-print-logo.png';
  const slug = product.slug || product._id.toString();

  return {
    title,
    description,
    keywords: [product.title, product.brand || 'Smart ePrint', categoryName, 'buy printer online', 'Smart ePrint'],
    alternates: {
      canonical: `https://www.smarteprint.com/product/${slug}`,
    },
    openGraph: {
      title: `${title} | Smart ePrint`,
      description,
      url: `https://www.smarteprint.com/product/${slug}`,
      images: [
        {
          url: imageUrl,
          alt: product.title,
...
export default async function ProductLayout({ children, params }) {
  ...
  if (product) {
    ...
    const imageUrls = product.images && product.images.length > 0
      ? product.images
      : ['https://www.smarteprint.com/smart-e-print-logo.png'];
    ...
    productSchema = {
      ...
      'brand': {
        '@type': 'Brand',
        'name': product.brand || 'Smart ePrint'
      },
      'offers': {
        '@type': 'Offer',
        'url': `https://www.smarteprint.com/product/${slug}`,
    ...
    breadcrumbSchema = {
      ...
      'itemListElement': [
        ...
        },
        {
          ...
          'position': 3,
          'name': product.title,
          'item': `https://www.smarteprint.com/product/${slug}`
        }
      ]
    };
  }
